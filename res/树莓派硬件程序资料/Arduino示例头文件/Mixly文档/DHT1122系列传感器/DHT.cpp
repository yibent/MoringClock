...
bool DHT::read(bool force)
{
  ...
  uint32_t cycles[80];
  {
    ...
    /*
    现在读取传感器发送的40位。每一位以50的形式发送
    微秒低脉冲，接着是可变长度的高脉冲。如果
    高脉冲约为28微秒，则为0，如果为~70微秒
    那就是1。我们测量初始50us低脉冲的循环计数
    并用它与高脉冲的循环计数进行比较以确定
    如果位是0（高状态循环计数<低状态循环计数），或
    1（高状态循环计数>低状态循环计数）。请注意，对于速度
    所有的脉冲都被读入一个阵列，然后在下一步进行检查。
    */
